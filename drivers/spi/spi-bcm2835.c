 ...
 *
 * The caller must ensure that the RX FIFO contains @count bytes
 * and that the DMA Enable flag in the CS register is set (such that a
 * read from the FIFO register receives 32-bit instead of just 8-bit).
 */
 ...
 *
 * The caller must ensure that the TX FIFO can accommodate @count bytes
 * and that the DMA Enable flag in the CS register is set (such that a
 * write to the FIFO register transmits 32-bit instead of just 8-bit).
 */
...
static irqreturn_t bcm2835_spi_interrupt(int irq, void *dev_id)
{
	...
	/*
	 * An interrupt is signaled either if RXR is set (RX FIFO >= 3/4 full)
	...
	} else if (bs->tx_len && cs & BCM2835_SPI_CS_DONE) {
 ...
 * be transmitted in 32-bit width, just as the following DMA transfer.
 */
